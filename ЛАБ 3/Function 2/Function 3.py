...
def KINO(movies, category):
    
    
    return [movie for movie in movies if movie["category"].lower() == category.lower()]
category_name = input("movie category: ")


movies_in_category = KINO(movies, category_name)

if movies_in_category:
    
    print(f"Movies in '{category_name}' category:")
    
    for movie in movies_in_category:
    
        print(f"{movie['name']} - IMDb: {movie['imdb']}")
else:
    print(f"No movies found in '{category_name}' category.")