...
# Функция для создания случайной еды с весом
# Функция для создания случайной еды с весом
class Food:
    def __init__(self):
        self.position = random_food()  # Генерация случайной позиции для еды
        self.weight = random.randint(1, 3)  # Генерация случайного веса еды от 1 до 3
        self.timer = time.time()  # Время появления еды
        self.lifetime = 5  # Время жизни еды в секундах (через 5 секунд еда исчезает)

    def move(self):
        # Если еда старше заданного времени, она исчезает и появляется на новом месте
        if time.time() - self.timer > self.lifetime:
            self.position = random_food()  # Генерация новой позиции для еды
            self.timer = time.time()  # Сбрасываем таймер

# Функция для случайной генерации еды
def random_food():
...
def game():
    ...
    food = Food()  # Инициализируем первую еду
    score = 0
    level = 1
    ...
    while running:
        ...
        # Обновляем состояние еды (таймер)
        food.move()
